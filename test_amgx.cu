...
int main(int argc, char** argv) {
    ...
    double *d_val     = nullptr, *d_rhs = nullptr, *d_zero = nullptr;
    ...
    double* h_zero = new double[n];
    cudaMalloc((void**)&d_zero, n * sizeof(double));
    std::fill(h_zero, h_zero + n, 0.0);
    cudaMemcpy(d_zero, h_zero, n * sizeof(double), cudaMemcpyHostToDevice);

    AMGX_vector_upload(x, n, 1, d_zero);
